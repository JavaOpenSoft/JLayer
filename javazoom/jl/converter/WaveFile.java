...
public class WaveFile extends RiffFile {
    ...
    /**
     * public int SeekToSample ( long SampleIndex ) { if ( SampleIndex >= NumSamples() ) { return DDC_INVALID_CALL; }
     * int SampleSize = (BitsPerSample() + 7) / 8; int rc = Seek ( pcm_data_offset + 8 + SampleSize * NumChannels() *
    ...
    /**
     * public int ReadStereoSample ( short[] LeftSampleData, short[] RightSampleData ) { int retcode = DDC_SUCCESS;
     * byte[] x = new byte[2]; short[] y = new short[2]; switch ( wave_format.data.nBitsPerSample ) { case 8: retcode =
     * Read ( x, 2 ); L[0] = (short) ( x[0] ); R[0] = (short) ( x[1] ); break;
     * <p>
