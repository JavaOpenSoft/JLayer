...
/**
 * The <code>AudioDeviceBase</code> class provides a simple thread-safe implementation of the <code>AudioDevice</code>
 * interface. Template methods are provided for subclasses to override and in doing so provide the implementation for
 * the main operations of the <code>AudioDevice</code> interface.
 *
 * @since 0.0.8
 * @author Mat McGowan
 */
...
public abstract class AudioDeviceBase implements AudioDevice {
    ...
    /**
     * Closes this audio device. If the device is currently playing audio, playback is stopped immediately without
    ...
    /**
     * Writes audio data to this audio device. Audio data is assumed to be in the output format of the decoder. This
     * method may return before the data has actually been sounded by the device if the device buffers audio samples.
     *
     ...
     * @throws JavaLayerException if the audio data could not be written to the audio device. If the audio device is not
     *                            open, this method does nthing.
     */
    ...
    /**
     * Waits for any buffered audio samples to be played by the audio device. This method should only be called prior to
     * closing the device.
     */
    public void flush() {
        if (isOpen()) {
